...
const files = await listTicketFiles(); let fails=[];
function score(md){
  let s=0;
...
for(const f of files){ const md=await read(f)||''; const sc=score(md); report.push(`- ${f}: ${sc}`); if(sc<min) fails.push(f) }
await write('artefacts/logs/ticket_quality_report.md',report.join('\n')+'\n');
if(fails.length){ console.error('Tickets below threshold:',fails); process.exit(1) } else { console.log('All tickets >=',min); process.exit(0) }
